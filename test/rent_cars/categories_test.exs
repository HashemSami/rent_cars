defmodule RentCars.CategoriesTest do
  use RentCars.DataCase

  alias RentCars.Categories
  alias RentCars.Categories.Category

  test "list_categories/0 return all categories" do
    assert Categories.list_categories() == []
  end

  test "create_category/1 with valid data" do
    attrs = %{description: "category desc", name: "HASH"}

    assert {:ok, %Category{} = category} = Categories.create_category(attrs)
    assert category.name == attrs.name
    assert category.description == attrs.description
  end

  test "create_category/1 with duplicated name" do
    attrs = %{description: "category desc", name: "HASH"}

    assert {:ok, %Category{} = _category} = Categories.create_category(attrs)
    assert {:error, changeset} = Categories.create_category(attrs)
    assert "has already been taken" in errors_on(changeset).name
  end

  test "create_category/1 without description field" do
    attrs = %{name: "Hash"}

    assert {:error, changeset} = Categories.create_category(attrs)
    assert "can't be blank" in errors_on(changeset).description
    assert %{description: ["can't be blank"]} = errors_on(changeset)
  end

  test "get_category/1 by id" do
    attrs = %{description: "category desc", name: "HASH"}

    {:ok, category} = Categories.create_category(attrs)

    assert Categories.get_category!(%{"id" => category.id}) == category
  end

  test "update_category/2 by id" do
    attrs = %{description: "category desc", name: "HASH"}

    {:ok, category} = Categories.create_category(attrs)

    {:ok, category_updated} = Categories.update_category(category, %{name: "SUV"})

    assert category_updated.name == "SUV"
  end

  test "delete_category/1 by id" do
    attrs = %{description: "category desc", name: "HASH"}

    {:ok, category} = Categories.create_category(attrs)

    {:ok, %Category{} = category_deleted} = Categories.delete_category(category)

    assert category.id == category_deleted.id
    assert_raise Ecto.NoResultsError, fn -> Categories.get_category!(%{"id" => category.id}) end
  end
end
